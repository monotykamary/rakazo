 ...
 * files that `destroyBot` cleans up per bot. Callers must claim the space with
 * `claimEmptySpaceDeletionForMember`, destroy any leftover team sandboxes, and
 * clear those providerRefs before invoking this so a failed `sandbox.destroy`
 * cannot lose the only persisted handle, and a failed delete cannot leave a
 * stale ref to a destroyed sandbox. Callers should surface
  ...
  return withTransactionRetry(() =>
    prisma.$transaction(
      async (tx) => {
        ...
        const planned = await assertEmptySpaceDeletable(tx, input);
        // Team computers left on the space may still hold control or execution
        // leases from a crashed run; release them so the sandbox teardown is
        // not fenced out by stale lease state on run-lease or remote machines.
        await tx.computerExecutionLease.deleteMany({
          where: { computer: { spaceId: input.spaceId } },
        });
