...
it.each([false, true])(
  "enforces fallback visibility with real SDK streams (hidden: %s)",
  async (hidden) => {
    ...
    try {
      ...
      const request: AgentRunRequest = {
        ...
        tools: [],
        model,
        assertModelAllowed: async (provider, modelId) =>
          assertModelVisible(
            { hide: hidden ? [{ provider: model.provider, model: "model-b" }] : [] },
            provider,
            modelId,
          ),
        modelRouting: {
      ...
      if (hidden) await expect(consume()).rejects.toThrow("hidden");
      else await consume();
      expect(calls).toEqual([
        { key: "Bearer fake-primary-key", model: "model-a" },
        { key: "Bearer fake-secondary-key", model: "model-a" },
        ...(hidden ? [] : [{ key: "Bearer fake-fallback-key", model: "model-b" }]),
      ]);
      if (!hidden) {
