...
export function oauthMaterialSecrets(material: OAuthMaterial): string[] {
  ...
  for (const [key, value] of Object.entries(material.headers ?? {})) {
    if (isAuthHeaderKey(key)) add(value);
    else if (isExplicitCredentialKey(key) && looksLikeSecretValue(value, true)) add(value);
    else if (isAmbiguousCredentialKey(key) && looksLikeSecretValue(value, false)) add(value);
  }
  for (const [key, value] of Object.entries(material.env ?? {})) {
    if (isExplicitCredentialKey(key) && looksLikeSecretValue(value, true)) add(value);
    else if (isAmbiguousCredentialKey(key) && looksLikeSecretValue(value, false)) add(value);
  }
...
function isAmbiguousCredentialKey(key: string): boolean {
  ...
  const normalized = key.toLowerCase().replace(/-/g, "_");
  return (
    /(?:^|_)token$/.test(normalized) && !/(timeout|ttl|max|count|type|mode|name)$/.test(normalized)
  );
}

function looksLikeSecretValue(value: string, allowNumeric: boolean): boolean {
  const trimmed = value.trim();
  if (!trimmed || COMMON_CONFIG_VALUES.has(trimmed.toLowerCase())) return false;
  return allowNumeric || !/^\d+(\.\d+)?$/.test(trimmed);
}
