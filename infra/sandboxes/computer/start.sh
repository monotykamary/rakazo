...
# full grace period before killing the container. Install the handler before any child starts
# so stops during startup are honoured too: forward SIGTERM and exit promptly.
XVFB_PID=""
shutdown() {
  trap - TERM INT
  if [[ -n "$XVFB_PID" ]]; then
