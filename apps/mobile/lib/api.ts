    ...
    if (!res.ok || parsed.error) {
      ...
      if (
        ...
        const recoveryOptions = {
          ...options,
          timeoutMs: null as number | null | undefined,
          signal: controller.signal,
...
export function applyMobileThreadEvent(
  ...
  if (event.type === "thread.message.created" || event.type === "thread.message.updated") {
    ...
    const existing = remaining.find((message) => message.id === messageId);
      ...
      replyToMessageId:
        event.payload?.replyToMessageId === undefined
          ...
          : event.payload.replyToMessageId
            ...
            : undefined,
      thumbsUp:
        typeof event.payload?.thumbsUp === "boolean" ? event.payload.thumbsUp : existing?.thumbsUp,
    };
    return {
      ...prev,
      cursor: event.seq ?? prev.cursor,
