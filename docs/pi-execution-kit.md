...
- `bun run pi:kit:vendor`: developer-only archive refresh after building and testing the sibling packages; update the lockfile and pinned identities together. Use `bun run pi:kit:vendor --only pi-hide-providers` to refresh only that archive and preserve all other hashes. Use `--source-root <prepared-checkouts>` to package clean committed snapshots without disturbing sibling work. Record their commits and preserve the generated content hashes; then run the installed-kit and RPC conformance tests.
