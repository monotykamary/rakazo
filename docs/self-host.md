...
Same as the README quick start: `.env` from `.env.example`, Postgres via Compose, `bun run sandbox:build`, `bun run dev`, then [http://127.0.0.1:5173](http://127.0.0.1:5173) (or `http://localhost:5173`; auth trusts both hosts with the same scheme and port). Electron: `bun run --filter @rakazo/desktop dev` while that stack is up, choosing **Another server** with that address. The desktop app's **This computer** option instead installs and runs the published images itself with Docker Compose (see [Published images](#published-images-no-checkout)), using port 45173 by default so it can run alongside `bun run dev`. If that port is occupied, the app selects and remembers another loopback port. The managed API gets a Docker-assigned loopback port; all desktop traffic uses the web origin.
...
On Windows, if an older clone with `core.autocrlf=true` leaves the computer pane hung on boot (`bash\r` in sandbox logs): from a clean worktree, set `git config core.autocrlf false`, run `git add --renormalize . && git checkout -- .`, then rebuild with `bun run sandbox:build`.
...
SANDBOX_PROVIDER=docker   # or none, e2b, daytona, box. Keep fake only for bun run test.
AGENT_RUNTIME=pi          # Keep scripted only for bun run test.
...
`GIT_SHA=$(git rev-parse HEAD)`, run `bun run --filter @rakazo/db migrate`, then restart API and worker.
